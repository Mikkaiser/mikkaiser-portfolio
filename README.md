...
- **"Ask about me".** A route handler calls Claude through the official `@anthropic-ai/sdk` with
...
`@anthropic-ai/sdk` · `simple-icons` · deployed on Vercel.
...
```bash
npm install
cp .env.example .env.local     # add ANTHROPIC_API_KEY to enable the chat locally
